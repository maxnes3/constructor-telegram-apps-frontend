...
export const TemplateDemo: FC<TemplateDemoProps> = ({ template }) => {
  ...
  const setIsOverPrototype = useConstructorStore(
    (state) => state.setIsOverPrototype,
  );
  const setPositionBehaviour = useConstructorStore(
    (state) => state.setPositionBehaviour,
  );
  const addTemplateAtPrototype = useConstructorStore(
    (state) => state.addTemplateAtPrototype,
  );

  const handleOnDrag = (e: DraggableEvent) => {
    const isInside = getIsInsidePrototype(e.target);

    setPositionBehaviour(positionBehaviour);
    setIsOverPrototype(isInside);
  };
  ...
  const handleOnStop = (e: DraggableEvent) => {
    ...
    setIsOverPrototype(false);
    setPositionBehaviour(null);
